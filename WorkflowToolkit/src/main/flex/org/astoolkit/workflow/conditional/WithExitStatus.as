...
package org.astoolkit.workflow.conditional
{
	...
	public class WithExitStatus extends BaseConditionalWorkflowExpression
	{

		[Inspectable( enumeration="complete,userCanceled,failed,timeOut,aborted," )]
		...
		override public function evaluate( inComparisonValue : * = null ) : Object
		{
			...
			if( code && code.length > 0 )
				return _context.variables.$exitStatus.code == code;

			if( condition )
			{
				if( condition is IConditionalExpressionGroup )
					IConditionalExpressionGroup( condition ).withValue = _context.variables.$exitStatus.code;
				return condition.evaluate( _context.variables.$exitStatus.code );
			}
			return false;
		}
	}
